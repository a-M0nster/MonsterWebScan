...
if (stripos($urlToScan, 'http') !== 0) $urlToScan = 'http://'.$urlToScan;
...
$crawlUrlFlag = false;
if (stristr($testCases, ' crawlurl ') !== false) $crawlUrlFlag = true;
...
if ($crawlUrlFlag) {
    ...
    updateStatus($db, "爬取 $urlToScan...", $testId);
    $log->lwrite('开始爬取');

    $crawler->go();
    $urlsFound = $crawler->urlsFound;
} else $urlsFound = array($urlToScan);
...
if (stristr($testCases, ' bannerdis ') !== false) {
    ...
    if (!$crawlUrlFlag) testHttpBannerDisclosure($urlsFound[0], $testId);
    else testHttpBannerDisclosure($siteBeingTested, $testId);
...
if (stristr($testCases, ' emailpdf ') !== false) updateStatus($db, "Scan is complete! The report has been emailed to you and is also in your scan history.", $testId);
else updateStatus($db, "Scan is complete! The report is in your scan history.", $testId);
